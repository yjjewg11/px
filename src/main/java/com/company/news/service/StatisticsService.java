...
public class StatisticsService extends AbstractServcice {
	 ...
	 * @return
	 */
	public PieStatisticsVo getUssBygroup(ResponseMessage responseMessage,String group_uuid) {
		//验证group合法性
	...
	/**
	 * 获取用户登陆时间统计
	 * user logintime Statistics
	 * @param responseMessage
	 * @return
	 */
	public PieStatisticsVo getUlsBygroup(ResponseMessage responseMessage,String group_uuid) {
		//验证group合法性
		if(!validateGroup(group_uuid, responseMessage))
			return null;
		...
		vo.setLegend_data("['"+lessthreeDay+"%的用户在3天内登陆过'"
				+ ",'"+lessOneWeek+"%的用户在1周内登陆过','"
				+greaterOneWeek+"%的用户在更早前']");
	 ...
	 * @return
	 */
	public boolean getCnsBygroup(ResponseMessage responseMessage) {
		return false;
	...
	/**
	 * 获取班级互动点击排行（按时间）
	 * class news top  Statistics
	 * @param responseMessage
	 * @return
	 */
	public boolean getCntsBygroup(ResponseMessage responseMessage) {
		return false;
	 ...
	 * @return
	 */
	private boolean validateGroup(String group_uuid,ResponseMessage responseMessage){
		// TEL格式验证
