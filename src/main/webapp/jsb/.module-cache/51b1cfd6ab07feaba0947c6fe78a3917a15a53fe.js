...
var ChooseCook_EventRow = React.createClass({displayName: "ChooseCook_EventRow", 
  ...
  render: function() {
    var event = this.props.event;
    var is_Checked=this.props.checkedCookuuid.indexOf(event.uuid)>-1;
    var className = is_Checked ? 'am-active' :
      event.disabled ? 'am-disabled' : '';

    return (
      React.createElement("tr", {id: "tr_chcook_"+event.uuid, className: className, onClick: this.tr_onClick.bind(this,"tr_chcook_"+event.uuid,"tb_cbox__chcook"+event.uuid)}, 
      React.createElement("td", null, 
      React.createElement("input", {type: "checkbox", alt: event.name, value: event.uuid, id: "tb_cbox__chcook"+event.uuid, name: "table_checkbox", checked: is_Checked?"checked":""})
      ), 
        React.createElement("td", null, event.name), 
        React.createElement("td", null, event.tel), 
        React.createElement("td", null, event.sex=="0"?"男":"女"), 
...
var ChooseCook_EventsTable = React.createClass({displayName: "ChooseCook_EventsTable",
//    	 getInitialState: function() {
//    		 	alert(this.props.group_uuid);
//    		    return this.props.group_uuid;
//    		  },
//	
	handleClick: function(m) {
		 if(this.props.handleClick){
			 if(m=="cancel"){
				 this.props.handleClick(m,$('#selectgroup_uuid_chcook').val());
				 return;
			 }
			 var uuids=null;
			 var names=null;
			 ...
			 this.props.handleClick(m,$('#selectgroup_uuid_chcook').val(),uuids,names);
		 }
	  },
	  handleChange_checkbox_all:function(){
		  $('input[name="table_checkbox"]').prop("checked", $("#id_checkbox_all_chcook")[0].checked); 
	  },
	  //
	  handleChange_selectgroup_uuid_chcook:function(){
		  var v=$('#selectgroup_uuid_chcook').val();
		//  this.setState(v);
		  w_ch_cook.reshowBygroup(v);
	  },
  render: function() {
	  var that=this;
    return (
  ...
  React.createElement("hr", null)
	  ...
	  React.createElement("div", {className: "am-form-group"}, 
      React.createElement("select", {id: "selectgroup_uuid_chcook", name: "group_uuid", "data-am-selected": "{btnSize: 'sm'}", value: this.props.group_uuid?this.props.group_uuid:"", onChange: this.handleChange_selectgroup_uuid_chcook}, 
      this.props.group_list.map(function(event) {
          return (React.createElement("option", {value: event.uuid}, event.company_name));
        })
      )
      ...
      React.createElement(AMUIReact_Table, React.__spread({},  this.props), 
        React.createElement("thead", null, 
          React.createElement("tr", null, 
          	React.createElement("th", null, 
            React.createElement("input", {type: "checkbox", id: "id_checkbox_all_chcook", onChange: this.handleChange_checkbox_all})
            ), 
            React.createElement("th", null, "昵称"), 
            React.createElement("th", null, "电话"), 
            React.createElement("th", null, "性别"), 
