...
var Basedatalist_EventRow = React.createClass({displayName: "Basedatalist_EventRow", 
  ...
  render: function() {
    ...
    return (
		 React.createElement("tr", null, 
        React.createElement("td", null, React.createElement("a", {href: "##", onClick: ajax_basedatalist_edit.bind(this, JSON.stringify(event))}, event.name)), 
...
var Basedatalist_EventsTable = React.createClass({displayName: "Basedatalist_EventsTable",
	  handleChange_checkbox_all:function(){
		  $('input[name="table_checkbox_basedatalist"]').prop("checked", $("#id_checkbox_all")[0].checked); 
	  },
  render: function() {
	  var that=this;
    return (
      ...
      React.createElement("button", {type: "button", onClick: ajax_basedatalist_button_handleClick.bind(this, "add_basedatalist",this.props.type), className: "am-btn am-btn-primary"}, "添加权限")
      )
    );
  }
});
