...
var ChooseClass_EventRow = React.createClass({displayName: "ChooseClass_EventRow", 
  ...
  render: function() {
    var event = this.props.event;
    var is_Checked=this.props.checkedClassuuid.indexOf(event.uuid)>-1;
    var className = is_Checked ? 'am-active' :
      event.disabled ? 'am-disabled' : '';
...
//	
  ...
  React.createElement("hr", null)
      ...
      React.createElement(AMUIReact_Table, React.__spread({},  this.props), 
        ...
        React.createElement("tbody", null, 
          this.props.events.map(function(event) {
            return (React.createElement(ChooseClass_EventRow, {event: event, checkedClassuuid: that.props.checkedClassuuid}));
          })
        )
      )
      )
