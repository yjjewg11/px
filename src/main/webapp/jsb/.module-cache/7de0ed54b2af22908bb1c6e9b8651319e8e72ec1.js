...
React.createElement("input", {type: "checkbox", value: event.uuid, name: "table_checkbox"})
  ...
  React.createElement("td", null, React.createElement("a", {href: "javascript:btn_click_cookbookPlan('edit','"+event.uuid+"')"}, event.plandate)), 
  React.createElement("td", null, event.morning), 
  React.createElement("td", null, event.afternoon), 
  React.createElement("td", null, event.create_useruuid)
) 
...
var CookbookPlan_EventsTable = React.createClass({displayName: "CookbookPlan_EventsTable",
	handleClick: function(m,uuid,classuuid) {
			 if(m=="add"){
				 btn_click_cookbookPlan(m,null,classuuid);
				 return;
			 }
			 var uuids=null;
			 $($("input[name='table_checkbox']")).each(function(){
			  ...
			  btn_click_cookbookPlan(m,$('#selectgroup_uuid').val(),uuids);
	  },
	  handleChange_checkbox_all:function(){
		  $('input[name="table_checkbox"]').prop("checked", $("#id_checkbox_all")[0].checked); 
	  },
