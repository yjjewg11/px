...
//btn_query_All_className:	显示-查询我的班级；
	...
	var obj=this.state;
	var arry_label=[];
    var selected=(React.createElement("div", null));
	...
	if(query_All_Type==false){
		selected=(React.createElement("div", null, 		  
    		...
    		React.createElement(AMUIReact.Selected, {id: "groupuuid", name: "groupuuid", onChange: this.handleChange_selectgroup, btnWidth: "200", data: obj.groupList, btnStyle: "primary", value: queryForm.groupuuid})		            
    	   ), 
  ...
  return (			
		  React.createElement("div", {"data-am-widget": "list_news", className: "am-list-news am-list-news-default"}, 
			...
			React.createElement(AMR_Panel, null, 
			 React.createElement(AMR_ButtonToolbar, null, 
	  		   ...
	  		   React.createElement(AMR_Button, {amStyle: "secondary", onClick: this.My_Phtot.bind(this)}, "查询已使用过的照片")
	  	      )
			 ), 
		  React.createElement("label", null, "图片总数："+this.state.totalCount+"张"), 
	    ...
	    React.createElement("div", {className: photoClassName}, 
	       React.createElement("label", null, "该班级相册暂未上传照片，请跳转班级相册上传新照片"), 
	       React.createElement(AMR_ButtonToolbar, null, 
		   React.createElement(AMR_Button, {amSize: "xs", amStyle: "secondary", onClick: this.btn_classPhtotItem.bind(this)}, "跳转班级相册")
		   )
		  ), 
	...
	for(var i=0;i<imgarry.length;i++){
	    ...
	    return (    		
		    React.createElement("div", {className: "am-comment-bd"}, 
		     React.createElement(Common_mg_Classnew_big_fn, {imgsList: imgphotoList, Penthat: this.props.Propsthat})
		    )
	    );
	  }
	});
		 ...
		 React.createElement("li", {id: "Common_mg_ClassNew_big_fn_item_"+ event.uuid, className: "G_class_phtoto_Img  G_ch_classNews_item "+className, title: event.uuid, onClick: that.div_onClick.bind(this,"Common_mg_ClassNew_big_fn_item_"+event.uuid,event)}, 			     						    
		  React.createElement("div", {className: "am-gallery-item"}, 
		   React.createElement("img", {src: o}), 
	        ...
	        React.createElement("label", null, "备注：【"+note_text+"】")
	       )
	      )		      
        	)
      })
 ...
 * 根据已使用照片相册请求数据绘制相册图片;
 * */
var Query_Myphoto_rect = React.createClass({displayName: "Query_Myphoto_rect",
 //数据初始化
 getInitialState: function() {
  return this.props.events; 
 },
//数据
 componentWillReceiveProps: function(nextProps) {	
	...
	var imgarry=this.state.data;
	    ...
	    return (    		
		    React.createElement("div", {className: "am-comment-bd"}, 
		     React.createElement(Common_Img_My_fn, {imgsList: imgarry, Penthat: this.props.Propsthat})
		    )
	    );
	  }
	});

var  Common_Img_My_fn  = React.createClass({displayName: "Common_Img_My_fn",
	//精品相册动态选择
	componentDidMount:function(){
		  if(G_photo_urlsSelectArry.length!=0){
			  var imglist=this.props.imgsList;
...
return (
 ...
 var  o = event.path;
	return (
		 ...
		 React.createElement("li", {id: "Common_Img_My_fn_item"+ event.uuid, className: "G_class_phtoto_Img  G_ch_classNews_item "+className, title: event.uuid, onClick: that.div_onClick.bind(this,"Common_Img_My_fn_item"+event.uuid,event)}, 			     						    
		  React.createElement("div", {className: "am-gallery-item"}, 
		   React.createElement("img", {src: o}), 
	        React.createElement("label", null, "创建时间：【"+event.create_time+"】")
	       )
	      )		      
        	)
      })
