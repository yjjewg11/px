  		   ...
  		   <AMR_ButtonToolbar>
  			  ...
  			  <div className="am-fl am-cf am-margin-bottom-sm am-margin-left-xs">
  			    <AMR_Button amStyle="secondary" onClick={this.handleClick.bind(this, "add")} >添加</AMR_Button>
						...
						<div className="am-fl am-cf am-margin-bottom-sm am-margin-left-xs">
						<AMR_Button amStyle="danger" onClick={this.handleClick_deleteUsersOfGroupRelation.bind(this)} >移除</AMR_Button>
						</div>
  				  <div className="am-fl am-cf am-margin-bottom-sm am-margin-left-xs">
  				  <input type="text" name="sutdent_name" id="sutdent_name"   placeholder="教师姓名"/>
  				  </div>
...
* @m：是启用还是禁用功能；"add"-添加  "edit"-修改；
* */
var Userinfo_add = React.createClass({
	 getInitialState: function() {
			// if(this.props.mygroup_uuids)this.props.formdata.group_uuid=this.props.mygroup_uuids;
		    return this.props.formdata;
		  },
  ...
  render: function() {
    ...
    return (
  		  ...
  		  <PxInput type="hidden" name="group_uuid"  value={o.group_uuid}/>
			  <PxInput type="hidden" name="uuid"  value={o.uuid}/>
		     <PxInput type="hidden" name="type"  value="1"/>
			   <div className= "am-form-group">

				 <label className={one_classDiv}>添加到机构:</label>
 			  <div className={two_classDiv}>
 				<PxInput  type="text" readOnly  value={this.props.groupName} placeholder=""/>
  					</div>
		   ...
		   {passwordDiv}
 	       <button type="button"  onClick={ajax_userinfo_addToOneGroupByAdmin}  className="am-btn am-btn-primary">提交</button>
		  </div>
		 </form>
    );
  }
